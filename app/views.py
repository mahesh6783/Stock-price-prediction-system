...
def  v_ticker(ticker_value,number_of_days):
    ...
    return ticker_value, number_of_days,plot_div


def payment_Expiration(u_id):     
    query1 = "SELECT status,expires_at FROM payment WHERE user_id_id=%s ORDER BY created_at DESC LIMIT 1;"
    c.execute(query1, (u_id,))
    data=c.fetchone()
    count=c.rowcount
    ...
    if count==0:
        ...
        if data[1]<today:
            query2="update payment set status='Expired' where expires_at=%s; " 
            c.execute(query2, (data[1],))
            db.commit()
        else:
            pass
    else:
...
def a_users(request):
    if 'admin_id' in request.session:        
        ...
        s1="select * from user where status='a';"
        c.execute(s1)
    
        data2=c.fetchall()

        s2="select * from user where status='p';"
        c.execute(s2)
        pending=c.rowcount
        data3=c.fetchall()

        s3="select * from user where status='s';"
        c.execute(s3)
        data4=c.fetchall()


        return render(request,'a_users.html',{"data1":data1,"data2":data2,"data3":data3,"data4":data4,"pending":pending})
    else:
        return redirect('../login/')
    
def a_request_user(request):
    if 'admin_id' in request.session:        
        s="select * from user where status='p';"
        c.execute(s)
        data1=c.fetchall()         
        return render(request,'a_request_user.html',{"data1":data1})
    else:
        return redirect('../login/')
        
    
def a_user_account_info(request):
    if 'admin_id' in request.session: 
        id_value = request.GET.get('id')
        query2 = "SELECT * FROM payment WHERE user_id_id=%s ORDER BY created_at DESC;"
        c.execute(query2, (id_value,))
        data2=c.fetchall()
        query="select * from User where id='"+id_value+"';"
        c.execute(query)
        data=c.fetchall()
        name=data[0][1]
        status=data[0][6]       
        #===graph==== 
        query1 = "select usage_time,date from u_usage where user_id='" + id_value + "';"
        c.execute(query1)
        data1 = c.fetchall()
        affected_rows = c.rowcount
        if affected_rows>=1:         
            df = pd.DataFrame(data1, columns=['usage_time','date'])             
            df['date'] = pd.to_datetime(df['date'])
            df.set_index('date', inplace=True)            
            df = df.resample('D').sum().fillna(0)                       
            usage_fig = go.Figure([go.Scatter(x=df.index, y=df['usage_time'])])
            usage_fig.update_xaxes(rangeslider_visible=True)
            usage_fig.update_layout(
                title='Usage Time',
                xaxis_title='Date',
                yaxis_title='Usage Time',
                paper_bgcolor="#14151b",
                plot_bgcolor="#14151b",
                font_color="white"
            )         
            graphic = plot(usage_fig, auto_open=False, output_type='div')
            return render(request,'a_user_account_info.html',{"data":data,'graphic': graphic,'data2':data2,'name':name,'status':status})
        else:
            return render(request,'a_user_account_info.html',{"data":data,'data2':data2,'name':name,'status':status})    
    else:
        return redirect('../login/')

    
def a_user_suspend(request):
    if 'admin_id' in request.session:        
        s="select * from user where status='s';"
        c.execute(s)
        data1=c.fetchall()         
        return render(request,'a_user_suspend.html',{"data1":data1})
    else:
        return redirect('../login/')
    
    
def a_request_user_approve(request):
    if 'admin_id' in request.session: 
        id_value = request.GET.get('id')
        p="a"
        query="update user set status=%s where id='"+id_value+"';"
        c.execute(query,(p))
        db.commit()
...
def a_request_user_suspend(request):
    if 'admin_id' in request.session: 
        id_value = request.GET.get('id')
        p="s"
        query="update user set status=%s where id='"+id_value+"';"
...
def a_request_user_reinstate(request):
    if 'admin_id' in request.session: 
        id_value = request.GET.get('id')
        p="a"
        query="update user set status=%s where id='"+id_value+"';"
        c.execute(query,(p))
        db.commit()
        return redirect('../a_user_suspend/')
    else:
        return redirect('../login/')
...
def admin_page(request):
    if 'admin_id' in request.session:
        query1 = "select*from user where status='a';"
        c.execute(query1)
        active_users=c.rowcount
        query2 = "select*from user where status='s';"
        c.execute(query2)
        suspend_users=c.rowcount
        query3 = "select*from user where status='p';"
        c.execute(query3)
        requested_users=c.rowcount
        n_users=requested_users

        query4 = "select*from user where ugender='male';"
        c.execute(query4)
        male=c.rowcount

        query5 = "select*from user where ugender='female';"
        c.execute(query5)
        female=c.rowcount

        query6="SELECT user_fd.*, user.u_image FROM user_fd JOIN user ON user_fd.u_id_id = user.id;"
        c.execute(query6)
        feedback=c.fetchall()
        ...
        labels = ['active users', 'requested users', 'suspended users']
        values = [active_users,requested_users,suspend_users]
        
        pie_fig = go.Figure(data=[go.Pie(labels=labels, values=values)])
        
        pie_fig.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
        # Generate Plotly div
        plot_div_pie = plot(pie_fig, auto_open=False, output_type='div')


        labels1 = ['Male users', 'Female users']
        values1 = [male, female]
 
        colors = ['blue', 'pink']
 
        pie_fig1 = go.Figure(data=[go.Pie(labels=labels1, values=values1, marker=dict(colors=colors))])
 
        pie_fig1.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")

        # Generate Plotly div
        plot_div_pie1 = plot(pie_fig1, auto_open=False, output_type='div')

        return render(request,'admin_page.html',{'active_users':active_users,'suspend_users':suspend_users,'requested_users':requested_users,'plot_div_pie':plot_div_pie,'plot_div_pie1':plot_div_pie1,'n_users':n_users ,'feedback': feedback,'total':total})
    else:
        return redirect('../login/')  
    

def a_user_wesite_usage(request):
    return render(request, 'a_user_wesite_usage.html')


def a_user_edit(request):
    if 'admin_id' in request.session: 
        id_value = request.GET.get('id')
        query="select * from User where id='"+id_value+"';"
        c.execute(query)
        data=c.fetchall()
        return render(request,'a_user_edit.html',{"data":data})
    else:
        return redirect('../login/') 
...
def user_edit_action(request):
    if 'admin_id' in request.session: 
        msg=""
        if(request.POST):
            id_int=request.POST['id']
            uname=request.POST['uname']
            ph=request.POST['phone']
            mail=request.POST['email']
            g=request.POST['gender']
            password=request.POST['pass']
            update_query = "UPDATE User SET uname=%s, uphone=%s, uemail=%s, ugender=%s, upwd=%s WHERE id=%s;"
            
            try:
                c.execute(update_query, (uname, ph, mail, g, password, id_int))
                db.commit()
                return redirect(f'../a_user_account_info?id={id_int}/')
...
def user_delete(request):
    if 'admin_id' in request.session:
        ...
        query="delete from payment where user_id_id='"+id_value+"';"
        c.execute(query)
        db.commit()

        query = "SELECT id FROM user_fd WHERE u_id_id = %s"
        c.execute(query, (id_value,))
        ids = c.fetchall()
  
        ids_to_delete = [row[0] for row in ids]

        if ids_to_delete:
            delete_query = "DELETE FROM user_fd_reply WHERE f_id_id IN (%s)" % ','.join(['%s'] * len(ids_to_delete))
            c.execute(delete_query, ids_to_delete)
            db.commit()


        query="delete from user_fd where u_id_id='"+id_value+"';"
        c.execute(query)
        db.commit()

        query="delete from u_usage where user_id='"+id_value+"';"
        c.execute(query)
        db.commit()

        query="delete from User where id='"+id_value+"';"
        c.execute(query)
        db.commit()
...
def a_user_feedback(request):
    if 'admin_id' in request.session: 
        
        query="select * from user_fd;"
        c.execute(query)
        data=c.fetchall()

        query1="select * from user_fd where category='good';"
        c.execute(query1)
        good=c.rowcount
        data1=c.fetchall()
        

        query2="select * from user_fd where category='bad';"
        c.execute(query2)
        bad=c.rowcount
        data2=c.fetchall()

        query3="select * from user_fd where category='intermediate';"
        c.execute(query3)
        intermediate=c.rowcount
        data3=c.fetchall()


        labels = ['good feedback', 'bad feedback', 'intermediate feedback']
        values = [good,bad,intermediate]
        ...
        return render(request,'a_user_feedback.html',{"data":data,"data1":data1,"data2":data2,"data3":data3,"plot_div_pie":plot_div_pie})
    else:
        return redirect('../login/')
    

def a_fd_collect(request):
    if 'user_id' in request.session:
        if request.POST:
            ...
            s = "INSERT INTO user_fd (ufd, date, time, u_id_id, uname, category) VALUES (%s, %s, %s, %s, %s, %s);"
            c.execute(s, (fd, date, current_time, u_id, name, classification))
            db.commit()
            return redirect('../a_user_feedback/')           
    else:
        return redirect('../login/')
    

def a_user_feedback_delete(request):
    if 'admin_id' in request.session:
        id_value = request.GET.get('id')

        s="delete from user_fd_reply where f_id_id='"+id_value+"';"
        c.execute(s)
        db.commit

        s2="delete from user_fd where id='"+id_value+"';"
        c.execute(s2)
        db.commit()
        return redirect('../a_user_feedback/')
    else:
...
def a_user_feedback_rply(request):
    if 'admin_id' in request.session:

        id_value =str(request.GET.get('id'))

        s="SELECT user_fd.ufd,user.u_image,user.uname FROM user_fd JOIN user ON user_fd.u_id_id = user.id WHERE user_fd.id ='"+id_value+"';"
        c.execute(s)
        data2=c.fetchall()


        a="SELECT user_fd_reply.*, COALESCE(user.u_image, 'admin.png') AS u_image, CASE WHEN user_fd_reply.user_id = 0 THEN 'vn.png' ELSE '' END AS verification_image FROM user_fd_reply LEFT JOIN user ON user_fd_reply.user_id = user.id WHERE user_fd_reply.f_id_id='"+id_value+"';"

        c.execute(a)
        data1=c.fetchall()  

        return render(request,'a_user_feedback_rply.html',{"data1":data1,"data2":data2,"id_value":id_value})  
         
       
    else:
        return redirect('../login/')

def a_fd_reply_collect(request):
    if 'admin_id' in request.session:
        if request.POST:
                fd = request.POST.get('fd', '')
                ...
                s = "INSERT INTO user_fd_reply (user_id,uname,ufd,date,time,f_id_id) VALUES (%s, %s, %s, %s, %s, %s);"
                c.execute(s, (u_id,name,fd,date,current_time,f_id))
                db.commit()
                return redirect(f'../a_user_feedback_rply?id={f_id}/')
...
def a_change_password(request):
    if 'admin_id' in request.session: 
        ...
        if request.method == 'POST':
            admin_id = str(request.session['admin_id'])
            old_pass = request.POST.get('old_pass', '')
            new_pass = request.POST.get('new_pass', '')
            c_pass = request.POST.get('c_pass', '')

            if admin_id:
                query1 = "SELECT aupwd FROM admin WHERE id =%s;"
                c.execute(query1,(admin_id,))
                data_p = c.fetchone()

                if data_p:
                    if old_pass == data_p[0]:
                        if new_pass == c_pass:
                            update_query = "UPDATE admin SET aupwd = %s WHERE id = %s;"
                            c.execute(update_query, (new_pass,admin_id))
                            db.commit()
                            return redirect('../admin_page/')
                        else:
                ...
                msg = "Admin ID not provided."
            
            return render(request, 'a_change_password.html', {'msg': msg})
        else:
...
def a_manage_model(request):
    if 'admin_id' in request.session:
        msg=""
        if(request.POST):
            if 'submit_form1' in request.POST:
                ticker=request.POST['nc']
                tname=ticker
                
                stock_symbol=ticker
                ticker_name=ticker+".h5"
                ...
                model = tf.keras.models.load_model(os.path.join(settings.MODELS,ticker_name))
            # ====================================================end load=====================================================================
            
            # ==============================================  deep learning  ==================================================================

                test_data = scaled_data[0:len(scaled_data), :]
                x_test = []
                ...
                # Plot actual vs predicted values for the last 20 days
                pred_df = pd.DataFrame({'Date': df.index[-len(predictions):], 'Prediction': predictions.flatten()})

                pred_fig1 = go.Figure([go.Scatter(x=pred_df['Date'], y=pred_df['Prediction'], name='Predicted')])
                pred_fig1.add_trace(go.Scatter(x=df.index[-len(predictions):], y=y_test.flatten(), mode='lines', name='Actual'))
                pred_fig1.update_xaxes(rangeslider_visible=True)
                pred_fig1.update_layout(title='Actual vs. Predicted Stock Prices (Last 30 Days)',
                                    paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
                plot_div_pred1= plot(pred_fig1, auto_open=False, output_type='div')
                

                return render(request, 'a_manage_model.html',{'plot_div_pred1':plot_div_pred1,'ticker':tname,'mse':mse,'mae':mae})
            elif 'submit_form2' in request.POST:
                if 'file' in request.FILES:
                    file=request.FILES['file']
                    upload_dir = os.path.join(settings.MODELS)
                    
                    image_path = os.path.join(upload_dir, file.name)
                    with open(image_path, 'wb+') as destination:
                        for chunk in file.chunks():
                            destination.write(chunk)
                    msg="sucessfully uploaded model"
                    return render(request, 'a_manage_model.html',{'msg':msg})

        return render(request, 'a_manage_model.html')
...
def get_user(id_value):
    ...
    query="select uname from User where id='"+id_value+"';"
    c.execute(query)
    data=c.fetchone()
    return  data[0] 
...
def user_home(request):
    ...
    if 'user_id' in request.session:
        id_value =str(request.session['user_id'])
        u_name=get_user(id_value)
        return render(request,'user_home.html',{'u_name':u_name})
    else:
        return redirect('../login/') 
...
def user_account(request):
    if 'user_id' in request.session:
        ...
        c.execute(query)
        data=c.fetchall()
        query1 = "SELECT id,amount,status,created_at,expires_at FROM payment WHERE user_id_id = %s order by created_at desc;"
        c.execute(query1, (id_value,))
        data1 = c.fetchall()
        return render(request,'user_account.html',{'data':data,'data1':data1})
    else:
        return redirect('../login/')
    

def user_user_edit(request):
    if 'user_id' in request.session:
        id_value = request.GET.get('id')
        query="select  * from User where id='"+id_value+"';"
        c.execute(query)
        data=c.fetchall()
        return render(request,'user_user_edit.html',{"data":data})         
    else:
        return redirect('../login/')
    
def user_user_edit_action(request):
    if 'user_id' in request.session: 
        msg=""
        if(request.POST):
            id_int=request.POST['id']
...
def user_change_pass(request):
    if 'user_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            if user_id:
                query1 = "SELECT upwd FROM user WHERE id =%s;"
                ...
                if data_p:
                    if old_pass == data_p[0]:
                        if new_pass == c_pass:
                            update_query = "UPDATE user SET upwd = %s WHERE id = %s;"
                            c.execute(update_query, (new_pass,user_id))
                            db.commit()
                            return redirect('../user_account/')
                        else:
                            msg = "New password and confirm password do not match."
    ...
    if 'user_id' in request.session:
        ...
        if request.method == 'POST':
            if 'image' in request.FILES:
                ...
                upload_dir = os.path.join(settings.MEDIA_ROOT, 'u_image')
                
                image_path = os.path.join(upload_dir, image.name)
                with open(image_path, 'wb+') as destination:
                    for chunk in image.chunks():
                        destination.write(chunk)
                update_query = "UPDATE User SET  u_image=%s WHERE id=%s;"            
             
                c.execute(update_query, (image.name,id_int))
                db.commit()
...
def user_model_select(request):
    if 'user_id' in request.session:
        
        return render(request,'user_model_select.html')
...
def scrape_yahoo_finance_news(ticker):
    ...
    for article in articles:
        title_elements = article.find_all('h3')
        summary_elements = article.find_all('p')
        link_elements = article.find_all('a')
        img_elements = article.find_all('img')
        
        for title_element, summary_element, link_element, img_element in zip(title_elements, summary_elements, link_elements, img_elements):
            title = title_element.text if title_element else "N/A"
            summary = summary_element.text if summary_element else "N/A"
            link = link_element['href'] if link_element and 'href' in link_element.attrs else "N/A"
            img_src = img_element['src'] if img_element and 'src' in img_element.attrs else "N/A"                      
            if 'stock' in summary  or 'market' in summary:
                news_list.append({
                    'Title': title,
                    'Summary': summary,
                    'Link': link,
...
def scrape_yahoo_finance_latest_news():
    url = f"https://finance.yahoo.com/topic/stock-market-news/"
    ...
    for item in soup.find_all('li', class_='js-stream-content'):
        ...
        title = h3.text if h3 else 'null'
        link = a['href'] if a else 'null'
        summary = p.text if p else 'null'
        img_src = img['src'] if img and 'src' in img.attrs else "N/A" 
        
        if title != 'mull' and link != 'null' and summary != 'null' and img_src != 'null':
            news_list1.append({'title': title, 'link': link, 'summary': summary, 'img_src':img_src})
...
def news_view(request):     
    if 'user_id' in request.session:        
        if(request.POST):
            ticker=request.POST['nc']
            news_list = scrape_yahoo_finance_news(ticker)
        ...
        latest_news=scrape_yahoo_finance_latest_news()
        context1={
                'latest_news':latest_news
            }
        return render(request, 'news.html',context1)
    else:
        return redirect('../login/') 
...
def user_payment(request):
    if 'user_id' in request.session:
        msg="your active payment is experied"
        u_id = str(request.session['user_id'])

        query1 = "SELECT status FROM payment WHERE user_id_id=%s ORDER BY created_at DESC LIMIT 1;"
        c.execute(query1, (u_id,))
        data=c.fetchone()
        count=c.rowcount
        ...
        if count!=0:
            if data[0] =="Expired":
                return render(request,'user_payment.html',{'msg':msg})
            
            elif data[0]=="active":
                return redirect('../user_lstm_search/')
        else:
...
def user_payment_collect(request):
    if 'user_id' in request.session:
        if(request.POST):
            days1 = int(request.POST['days'])
            amount = float(request.POST['amount'])

            expire = datetime.date.today() + timedelta(days=days1)
            t_id = random.randint(100000000000, 999999999999)
            u_id = str(request.session['user_id'])

            # Fetch user's name from the database
            query1 = "SELECT uname FROM User WHERE id=%s;"
            c.execute(query1, (u_id,))
            name = c.fetchone()[0]  

            status = "active"
           
            payment_day = datetime.datetime.now()
            expire_d = expire.strftime("%Y-%m-%d")

             
            query = "INSERT INTO payment (uname, amount, status, transaction_id, created_at, expires_at, user_id_id) VALUES (%s, %s, %s, %s, %s, %s, %s);"
            values = (name, amount, status, t_id, payment_day, expire_d, u_id)
...
def feed_back(request):
    if 'user_id' in request.session:
        u_id =str(request.session['user_id'])
        # s="SELECT user_fd.*,user.u_image from user_fd,user where user_fd.u_id_id=user.id"
        s="SELECT user_fd.*, user.u_image, COUNT(user_fd_reply.id) AS num_replies FROM user_fd JOIN user ON user_fd.u_id_id = user.id LEFT JOIN  user_fd_reply ON user_fd.id = user_fd_reply.f_id_id GROUP BY  user_fd.id, user.u_image;"

        c.execute(s)
        data1=c.fetchall()


        s1="SELECT * from user_fd where u_id_id='"+u_id+"';"
        c.execute(s1)
        data2=c.fetchall()

        return render(request,'feed_back.html',{"data1":data1,"data2":data2})
    else:
        return redirect('../login/')
    
def fd_collect(request):
    if 'user_id' in request.session:
        if request.POST:
            fd = request.POST.get('fd', '')  
            ...
            query = "SELECT uname FROM User WHERE id=%s;"
            ...
            sentiment_scores = sid.polarity_scores(fd)
            ...
            c.execute(s, (fd, date, current_time, u_id, name, classification))
            db.commit()
            return redirect('../feed_back/')
            

    else:
        return redirect('../login/')
def feed_back_delete(request):
    if 'user_id' in request.session:
        id_value =str(request.GET.get('id'))
...
def user_fb_reply(request):
    if 'user_id' in request.session:
        ...
        c.execute(a)
        data1=c.fetchall()                        
        return render(request,'user_fb_reply.html',{"data1":data1,"data2":data2,"id_value":id_value})    
    else:
        return redirect('../login/')
    
def collect_reply(request):
    if 'user_id' in request.session:
        if request.POST:
            fd = request.POST.get('fd', '')
            f_id = request.POST.get('fd_id', '')   
            ...
            return redirect(f'../user_fb_reply?id={f_id}/')
    else:
        return redirect('../login/')
...
def u_account(request):
    if 'user_id' in request.session:
        id_value = request.GET.get('id')
        query="select uname,uphone,uemail,ugender from User where id='"+id_value+"';"
        c.execute(query)
        data=c.fetchall()
        return render(request,'u_account.html',{"data":data})         
    else:
        return redirect('../login/') 
...
def generate_pdf(request):
    ...
    id_value = request.GET.get('id')
    query2 = "SELECT * FROM payment WHERE id=%s;"
    c.execute(query2, (id_value,))
    data=c.fetchone()
    status =str( data[3])  
    name = str(data[1])  
    amount=str(data[2])
    transaction_id = str(data[4])  
    payment_date = str(data[5])   
    Expiration_date = str(data[6])   
    ...
    company_name = "Stock Price Prediction"
    ...
    spacer1 = Spacer(1, 24)
    spacer2 = Spacer(1, 12)    
    receipt_elements = [company_name_paragraph, spacer1, table] + [spacer2]   
    doc.build(receipt_elements)    
    buffer.seek(0) 
...
def login(request):
    ...
    if  'admin_id' in request.session:
        ...
        if(request.POST):
            ...
            query = "SELECT id FROM User WHERE uemail = %s AND upwd = %s AND status='a'"
            c.execute(query, (email, pwd))
            data = c.fetchall()
            if data:
                ...
                u_id=str(request.session['user_id'])
                payment_Expiration(u_id)
                return redirect('../user_home/')   
            
            else:
                query1 = "SELECT id FROM admin WHERE aemail = %s AND aupwd = %s"
                c.execute(query1, (email, pwd))
                data1 = c.fetchall()

                if data1:
                    request.session['admin_id'] = data1[0][0]  
                    return redirect('../admin_page/') 
...
def log_out(request):
    if 'user_id' in request.session:
        ...
        login_t=request.session['start_time']
        login_time = datetime.datetime.strptime(login_t,"%H:%M:%S")
        current_t= datetime.datetime.now().strftime("%H:%M:%S")
        current_time = datetime.datetime.strptime(current_t,"%H:%M:%S")
        total_t=current_time-login_time
        total_time=total_t.total_seconds()
        u_id=str(request.session['user_id'])
        current_date = datetime.date.today()
        today = str(current_date.strftime("%Y-%m-%d"))
        da="a"
        query="select date,usage_time from u_usage where user_id='"+u_id+"' and date='"+today+"';"
        c.execute(query)
        d=c.fetchall()
        affected_rows = c.rowcount
        ...
        if affected_rows>=1:
            ...
            if da==today:
                ...
                total_time_u=str(d[0][1]+total_time)
                
                query1="update u_usage set usage_time='"+total_time_u+"' where user_id='"+u_id+"' and date='"+today+"';"
                c.execute(query1)
                db.commit()
        else:
            total_time_i=str(total_time)
            query2 = "INSERT INTO  u_usage (date, usage_time, user_id) VALUES ('"+today+"','"+total_time_i+"','"+u_id+"');"
...
#         mail=request.POST['email']
#         g=request.POST['gender']
#         hashed_password = request.POST['pass']
#         # salt = bcrypt.gensalt()
...
#         query = """
#         INSERT INTO User (uname, uphone, uemail, ugender, upwd, status, u_image)
#         VALUES (%s, %s, %s, %s, %s, %s, %s)
#         """
#         params = (uname, ph, mail, g, hashed_password, status, user_image)        
#         try:
...
def register(request):
    ...
    if request.POST:
        ...
        query_check_email = "SELECT COUNT(*) FROM User WHERE uemail = %s"
        c.execute(query_check_email, (mail,))
        result = c.fetchone()
        ...
        if result[0] > 0:
            ...
            try:
                c.execute(query, params)
                db.commit()
                s="Registration request is Success please wait"
            except:
                msg = "Sorry, registration error"
...
def r_s(request):
    ...
    return render(request,'r_s.html')
    return redirect('/')


def user_lstm_search(request):
    return render(request, 'user_lstm_search.html', {})


def user_prediction_LSTM(request, ticker_value, number_of_days):
    
    ticker_value, number_of_days,plot_div=v_ticker(ticker_value, number_of_days)

    stock_symbol = ticker_value
    ...
    if ticker_value=='GOOGL':
        ticker_name='GOOGL.h5'

    elif ticker_value=='AAPL':
        ticker_name='AAPL.h5'

    elif ticker_value=='AMZN':
        ticker_name='AMZN.h5'

    elif ticker_value=='META':
        ticker_name='META.h5'
    
    elif ticker_value=='NVDA':
        ticker_name='NVDA.h5'

    elif ticker_value=='QCOM':
        ticker_name='QCOM.h5'

    elif ticker_value=='TSLA':
        ticker_name='TSLA.h5'
     
    #==================================================loading LSTM trained model======================================================  
    model = tf.keras.models.load_model(os.path.join(settings.MODELS,ticker_name))
    # ====================================================end load=====================================================================
    ...
    last_30_days = df.filter(['Close'])[-30:].values
    ...
    next_daty= future_prediction[0][0]

    # Make predictions for the next 10 days
    future_data = df.filter(['Close'])
    last_60_days = future_data[-60:].values
    ...
    for i in range(number_of_days):
        ...
        X_test = np.append(X_test[:, 1:, :], [[predicted_price[0]]], axis=1)
    ...
    for i in range(0,ticker.shape[0]):
        if ticker.Symbol[i] == to_search:
            ...
            break
    return render(request, 'user_prediction_LSTM.html', {'plot_div': plot_div, 
                                                     'next_daty':next_daty,
